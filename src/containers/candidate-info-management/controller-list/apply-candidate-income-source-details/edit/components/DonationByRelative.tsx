...
import { EARNING_FROM_DONATION_BY_RELATIVES } from '@validations/candidate-info-management/operator/income-source-details/incomeSourceDetailsValidation';
...
import { FUNDING_SOURCES, donationByRelativeDefaultValues } from '../constants';

export const DonationByRelative = ({ editHandler, deleteHandler }: any) => {
  ...
  const {
    fields: donationByRelativeFields,
    append: donationByRelativeAppend,
    remove: donationByRelativeRemove,
  } = useFieldArray({
    name: 'donationByRelative',
  });
  ...
  return (
    <>
      {donationByRelativeFields.map((item: any, index) => {
        return (
          <div key={item.id}>
            <FormInput
              title="INCOME_SOURCE_DETAILS.POTENTIAL_AMOUNT"
              registerName={`donationByRelative.${index}.${EARNING_FROM_DONATION_BY_RELATIVES.POTENTIAL_AMOUNT}`}
              placeholder={t('PLACEHOLDER.ENTER')}
            />
            <FormInput
              title="INCOME_SOURCE_DETAILS.RELATIVE_NAME"
              registerName={`donationByRelative.${index}.${EARNING_FROM_DONATION_BY_RELATIVES.RELATIVE_NAME}`}
              placeholder={t('PLACEHOLDER.ENTER')}
            />
            <FormInput
              title="INCOME_SOURCE_DETAILS.RELATIVE_ADDRESS"
              registerName={`donationByRelative.${index}.${EARNING_FROM_DONATION_BY_RELATIVES.RELATIVE_ADDRESS}`}
              placeholder={t('PLACEHOLDER.ENTER')}
            />
            <FormInput
              title="INCOME_SOURCE_DETAILS.RELATION"
              registerName={`donationByRelative.${index}.${EARNING_FROM_DONATION_BY_RELATIVES.RELATION}`}
              placeholder={t('PLACEHOLDER.ENTER')}
            />
            <FormInput
              title="INCOME_SOURCE_DETAILS.RELATIVE_INCOME_SOURCE"
              registerName={`donationByRelative.${index}.${EARNING_FROM_DONATION_BY_RELATIVES.RELATIVE_INCOME_SOURCE}`}
              placeholder={t('PLACEHOLDER.ENTER')}
            />
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    editHandler(
                      index,
                      item.idx,
                      FUNDING_SOURCES.RELATIVE_DONATION,
                    );
                  }}
                >
              ...
              <Button
                ...
                onClick={() => {
                  if (item.idx) {
                    ...
                    donationByRelativeRemove(index);
                  }
                }}
              >
      ...
      <div className="py-8 mb-8 rounded-4 bg-white">
        <Button
          ...
          htmlType="button"
          onClick={() =>
            donationByRelativeAppend({
              ...donationByRelativeDefaultValues,
            })
          }
        >
