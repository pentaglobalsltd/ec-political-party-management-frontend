...
function Actions({
  ...
  const handleEditButton = (row: any, isEditableAROOP = false) => {
    navigate({
      pathname: ROUTES.EDIT_SYSTEM_USER(row?.userId),
      search: `action=edit&type=${USER_PROFILE_LIST_TYPE.ELECTION}`.concat(
        isEditableAROOP ? '&userType=ARO_OP' : '',
      ),
    });
  };
  ...
  return (
    <>
      <div className="d-flex gap-6">
        <div className="pointer" onClick={() => handleEditButton(row)}>
        ...
        {row?.userTypeCode === '1010' && (
          <div className="pointer" onClick={handleOperatorListModal}>
