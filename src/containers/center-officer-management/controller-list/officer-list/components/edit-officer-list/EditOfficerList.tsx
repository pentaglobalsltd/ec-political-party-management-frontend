...
import { useGetOfficer } from '@hooks/center-officer-management/controller-list/officer-list/useGetOfficer';
import { useUpdateOfficer } from '@hooks/center-officer-management/controller-list/officer-list/useUpdateOfficer';
...
const EditOfficerList = () => {
  ...
  const { officer, getOfficer } = useGetOfficer();
  const { updateOfficer, success } = useUpdateOfficer();
  ...
  const onSubmit: SubmitHandler<CreateOfficerListDataType> = (data: any) => {
    const modifiedData = {
      ...
      currentWorkPlaceAddress: data.currentWorkPlaceAddress,
      permanentAddress: data.permanentAddress,
      userTypeCode: data.userTypeCode,
      isItKnowledge: data.isItKnowledge,
      isVotedInEvm: data.isVotedInEvm,
      isOnMaternityLeave: data.isOnMaternityLeave,
      isOnEducationLeave: data.isOnEducationLeave,
      isOnPrn: data.isOnPrn,
    };

    updateOfficer(id as string, modifiedData);
  };

  return (
    <div className="container-96 mb-24">
      <Header
        headerText={{ header: t('OFFICER_LIST.EDIT_OFFICER_FORM_TITLE') }}
        breadcrumbs={editOfficerListBreadcrumbs(t)}
...
export default EditOfficerList;
