...
import { fetchPollingCenterSummaryRO } from '@api/result-management/electoral-process/results/get-polling-center-summarya-ro';
import {
...
export const useGetPollingCenterSummaryRO = () => {
  ...
  const getPollingCenterSummary = async (scheduleId: number | string) => {
    setLoading(true);
    try {
      const response = await fetchPollingCenterSummaryRO(scheduleId);
