...
import { generatePollingCenterDetailsSummaryPdf } from '@api/miscellaneous/documents/pdf-generator';
...
export const usePollingCenterDetailsSummaryPdfGenerator = () => {
  ...
  const downloadFileName = 'gazetted-center-list-summary.pdf';

  const getBufferData = async (params: PollingCenterDetailsParams) => {
    setLoading(true);
    try {
      const response = await generatePollingCenterDetailsSummaryPdf(
        params,
        keycloak?.token,
      );
  ...
  const downloadPollingCenterDetailsSummary = () => {
    if (!pdfBuffer) return;
